...
def init_sphere():
    sphere_radis = 1
    r = sphere_radis * ti.random(float)
    angle = 2 * pi * ti.random(float)
    x = r * ti.sin(angle)
    y = r * ti.cos(angle)
    z_limit = ti.sqrt(sphere_radis ** 2 - x ** 2 - y ** 2)
    z = 2 * z_limit * ti.random() - z_limit
    return x, y, z
@ti.kernel
def init():
    ...
    for i in range(n_particles):
        x,y,z = init_sphere()
        F_x[i] = [
            x * 0.13 + 0.8 - 0.15 * (i // group_size),
            y * 0.13 + 0.8 - 0.15 * (i // group_size),
            z * 0.13 + 0.8 - 0.15 * (i // group_size)
        ]
